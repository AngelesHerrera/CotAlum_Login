...
class Cotizar: AppCompatActivity(), AdapterView.OnItemSelectedListener {
        ...
        cotizarButton.setOnClickListener {
            if (AltoEditText.text.toString().equals("")){
                ...
                db.collection("Precios").document(email).get().addOnSuccessListener {
                    ...
                    totalVidrio = ((AnchoEditText.text.toString().toInt())*(AltoEditText.text.toString().toInt())*vidrio)
                    var total= (res + res1 + res2 + res3 + res4 + res5 + res6 + res7)
                    var total1 = ((total*.65) + total) + totalVidrio
    ...
    private fun redondear(){
        ...
        if (jamba<=50){
            ...
            Jamba= 6.0
        }else Jamba = jamba.toDouble()
    }

    override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        if (position==0) {
            vidrio = 450
            //totalVidrio = (vidrio * ResultVidrio)
        }else if (position==1) {
            vidrio = 740
            //totalVidrio = (vidrio * ResultVidrio)
        }else if (position==2) {
